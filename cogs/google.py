...
class Google:
    ...
    async def i(self, ctx, *, query):
        ...
        async with aiohttp.get("https://www.googleapis.com/customsearch/v1?q=" + urllib.parse.quote_plus(query) + "&start=" + '1' + "&key=" + config['google_api_key'] + "&cx=" + config['custom_search_engine'] + "&searchType=image") as resp:
            if resp.status != 200:
                return await self.bot.send_message(ctx.message.channel, 'Google failed to respond.')
            result = json.loads(await resp.text())
            try:
                result['items']
            except:
                return await self.bot.send_message(ctx.message.channel, 'There were no results to your search. Use more common search query or make sure you have image search enabled for your custom search engine.')
