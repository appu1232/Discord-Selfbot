...
class Misc:
    ...
    async def stats(self, ctx):
        ...
        if embed_perms(ctx.message):
            ...
            await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + msg)
        await self.bot.delete_message(ctx.message)
        ...
        **All properties and the syntax (put your custom stuff in place of the <> stuff):
        ...
        if msg:
            if embed_perms(ctx.message):
                ...
                await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'No embed permissions in this channel.')
        else:
        ...
        with open('settings/optional_config.json', 'r+') as fp:
            ...
            if color:
                try:
                    ...
                    return await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Invalid color.')
                opt['embed_color'] = color
                await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Successfully set color for embeds.')
            else:
                opt['embed_color'] = ""
                await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Set default embed color off for embed command. You will now need to specify the color parameter if you want your embed to be colored when using the embed command.')
    ...
    async def game(self, ctx, *, game: str = None):
        ...
        if game:
            ...
            if ' | ' in game:
                await self.bot.send_message(ctx.message.channel,
                                            self.bot.self.bot.bot_prefix + 'Input interval in seconds to wait before changing to the next {} (``n`` to cancel):'.format(
                                                status_type.lower()))
                ...
                if reply.content.lower().strip() == 'n':
                    return await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Cancelled')
                    ...
                    if interval >= 10:
                        ...
                        if len(games) != 2:
                            await self.bot.send_message(ctx.message.channel,
                                                        self.bot.self.bot.bot_prefix + 'Change {} in order or randomly? Input ``o`` for order or ``r`` for random:'.format(
                                                            status_type.lower()))
                            ...
                            if s.content.strip() == 'r' or s.content.strip() == 'random':
                                await self.bot.send_message(ctx.message.channel,
                                                            self.bot.self.bot.bot_prefix + '{status} set. {status} will randomly change every ``{time}`` seconds'.format(
                        ...
                        if loop_type == 'ordered':
                            await self.bot.send_message(ctx.message.channel,
                                                        self.bot.self.bot.bot_prefix + '{status} set. {status} will change every ``{time}`` seconds'.format(
                                                            status=status_type, time=reply.content.strip()))
                        ...
                        return await self.bot.send_message(ctx.message.channel,
                                                           self.bot.self.bot.bot_prefix + 'Cancelled. Interval is too short. Must be at least 10 seconds.')
            ...
            # Set game if only one game is given.
                ...
                if is_stream and '=' in game:
                    g, url = game.split('=')
                    await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Stream set as: ``Streaming %s``' % g)
                    await self.bot.change_presence(game=discord.Game(name=g, type=1, url=url))
                else:
                    await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Game set as: ``Playing %s``' % game)
        ...
        # Remove game status.
            ...
            await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Set playing status off')
    ...
    async def avatar(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if avi_config['password'] == '':
                return await self.bot.send_message(ctx.message.channel,
                                                   self.bot.self.bot.bot_prefix + 'Cycling avatars requires you to input your password. Your password will not be sent anywhere and no one will have access to it. '
            ...
            if avi_config['interval'] != '0':
                ...
                await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Disabled cycling of avatars.')
            else:
                if os.listdir('avatars'):
                    await self.bot.send_message(ctx.message.channel,
                                                self.bot.self.bot.bot_prefix + 'Enabled cycling of avatars. Input interval in seconds to wait before changing avatars (``n`` to cancel):')
                    ...
                    if interval.content.lower().strip() == 'n':
                        ...
                        return await self.bot.send_message(ctx.message.channel,
                                                           self.bot.self.bot.bot_prefix + 'Cancelled. Interval is too short. Must be at least 1800 seconds (30 minutes).')
                    else:
                        avi_config['interval'] = int(interval.content)
                    if len(os.listdir('avatars')) != 2:
                        await self.bot.send_message(ctx.message.channel,
                                                    self.bot.self.bot.bot_prefix + 'Change avatars in order or randomly? Input ``o`` for order or ``r`` for random:')
                        ...
                        if cycle_type.content.strip() == 'r' or cycle_type.content.strip() == 'random':
                            await self.bot.send_message(ctx.message.channel,
                                                        self.bot.self.bot.bot_prefix + 'Avatar cycling enabled. Avatar will randomly change every ``%s`` seconds' % interval.content.strip())
                            loop_type = 'random'
                        else:
                            loop_type = 'ordered'
                    else:
                    ...
                    if loop_type == 'ordered':
                        await self.bot.send_message(ctx.message.channel,
                                                    self.bot.self.bot.bot_prefix + 'Avatar cycling enabled. Avatar will change every ``%s`` seconds' % interval.content.strip())
                    ...
                    await self.bot.send_message(ctx.message.channel,
                                                self.bot.self.bot.bot_prefix + 'No images found under ``avatars``. Please add images (.jpg .jpeg and .png types only) to that folder and try again.')
    ...
    async def password(self, ctx, *, msg):
        ...
        await self.bot.delete_message(ctx.message)
        return await self.bot.send_message(ctx.message.channel,
                                           self.bot.self.bot.bot_prefix + 'Password set. Do ``>avatar`` to toggle cycling avatars.')
    ...
    async def setavatar(self, ctx, *, msg):
        ...
        if url:
            ...
            await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Could not find image.')
    ...
    async def ping(self, ctx):
        ...
        if embed_perms(ctx.message):
            ...
            await self.bot.send_message(ctx.message.channel,
                                        self.bot.self.bot.bot_prefix + '``Response Time: %s ms``' % str(ping.microseconds / 1000.0))
    ...
    async def quote(self, ctx, *, msg: str = None):
        ...
        if result:
            ...
            await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'No quote found.')
        ...
        with open('settings/optional_config.json', 'r+') as fp:
            ...
            if txt:
                if txt.strip() == 'idle':
                    ...
                    return await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + info)
            else:
                return await self.bot.send_message(ctx.message.channel, self.bot.self.bot.bot_prefix + 'Invalid status. %s' % info)
            fp.seek(0)
            fp.truncate()
            json.dump(opt, fp, indent=4)
            await self.bot.send_message(ctx.message.channel,
                                        self.bot.self.bot.bot_prefix + 'Set default afk status. You will now appear as ``{}`` when not on Discord.'.format(
                                            opt['default_status']))
